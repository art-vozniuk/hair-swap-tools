...
class StableHair:
    def __init__(self, source_image: str, reference_image: str, device=None, weight_dtype=torch.float16) -> None:
        ...
        ### Load controlnet
        unet = UNet2DConditionModel.from_pretrained(self.pretrained_model_path, subfolder="unet").to(self.device)
        controlnet = ControlNetModel.from_unet(unet).to(self.device)
        _state_dict = torch.load(os.path.join(self.pretrained_folder, self.controlnet_path), map_location="cpu")
        ...
        self.hair_encoder = ref_unet.from_pretrained(self.pretrained_model_path, subfolder="unet").to(self.device)
        _state_dict = torch.load(os.path.join(self.pretrained_folder, self.encoder_path), map_location="cpu")
        ...
        _state_dict = torch.load(os.path.join(self.pretrained_folder, self.adapter_path), map_location="cpu")
        self.hair_adapter.load_state_dict(_state_dict, strict=False)

        ### load bald converter
        bald_converter = ControlNetModel.from_unet(unet).to(self.device)
        _state_dict = torch.load(self.bald_converter_path, map_location="cpu")
